...
package com.Litterfeldt.AStory.scrollBar;
...
public class ColorPicker extends View {
	 ...
	 * </p>
	 */
	private static final int[] COLORS = new int[] { 0xff00ffff,0xff00ffff,0x00000000 };
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		mAngle = (float) 0;//(-Math.PI / 2);

        float[] in =  {(float)0,(float)0,(float)0};
		Shader s =  new SweepGradient(0, 0, COLORS,in );
    ...
    public void setProgress(float progress){
        float circomfrence = 2*(float)Math.PI;
        float hundred = (float) 100.00;
        mAngle = circomfrence* progress/hundred;
        float percentprogress =  progress/hundred;
        float[] in =  {(float)0,(float)percentprogress,(float)percentprogress};
        Shader s =  new SweepGradient(0, 0, COLORS,in );
